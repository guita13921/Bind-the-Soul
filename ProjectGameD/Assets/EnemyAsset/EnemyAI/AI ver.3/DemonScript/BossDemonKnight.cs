...
namespace SG
{
    ...
    public class DemonKnightBoss : MonoBehaviour
    {
        ...
        private List<BossAction> SpecialCombo06 = new List<BossAction> { BossAction.CallEnemy, BossAction.OneHandCast01 };
        ...
        [SerializeField] EnemyStat enemyHealth;
        ...
        void Awake()
        {
            BossAnimation = GetComponent<BossDemon_Animation>();
        }

        void Start()
        {
            if (!player) player = GameObject.FindWithTag("Player").transform; // Find player if not set
            enemyHealth = GetComponent<EnemyStat>();
